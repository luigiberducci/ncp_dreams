...
def create_log_dir(model_name, base_dir=pathlib.Path('log'), hparams=Dict):
    desc = '__'.join([f'{param.name}_{value}' for param, value in hparams.items()])
    outdir = base_dir / f'{model_name}_{desc}_{time.time()}'
...
def create_model(model_name: str, hparams: Dict, motor_neurons: int = 1):
    ...
    if motor_neurons == 1:
        model = SteeringImitationModel(name=model_name, conv_filters=filters, conv_kernels=kernels,
                                       conv_strides=strides,
                                       motor_neurons=motor_neurons)
    else:
        model = MotorSteeringImitationModel(name=model_name, conv_filters=filters, conv_kernels=kernels,
                                            conv_strides=strides,
                                            motor_neurons=motor_neurons)
    return model
