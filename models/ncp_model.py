...
class ConvNCP(tools.Module):
    ...
    def __init__(self, name, logdir,
                 conv_filters: List[int], conv_kernels: List[int], conv_strides: List[int], encoded_dim: int,
                 inter_neurons: int, command_neurons: int, motor_neurons: int, sensory_fanout: int = 6,
                 inter_fanout: int = 4, recurrent_command_synapses: int = 6, motor_fanin: int = 4,
                 opt_lr: float = 1e-3):
        self._name = name
        ...
        dict_config = {
            'filters': conv_filters, 'kernels': conv_kernels,
            'strides': conv_strides, 'encoded_dim': encoded_dim,
            'inter_neurons': inter_neurons, 'command_neurons': command_neurons,
            'motor_neurons': motor_neurons, 'sensory_fanout': sensory_fanout,
            'inter_fanout': inter_fanout, 'recurrent_command_synapses': recurrent_command_synapses,
            'motor_fanin': motor_fanin}
        config_file = logdir / 'checkpoints/conv_ncp.yml'
        with open(config_file, 'w') as file:
            yaml.dump(dict_config, file)
            print(f'[Info] Stored model configuration in {config_file}')
...
class NCP(tools.Module):
    ...
    def __init__(self, inter_neurons, command_neurons, motor_neurons,
                 sensory_fanout, inter_fanout, recurrent_command_synapses, motor_fanin):
...
class ConvHead(tools.Module):
    def __init__(self, filters, kernels, strides, encoded_dim=32, activation='relu'):
